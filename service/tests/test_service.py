...
def test_txt_default_is_one_ayah_per_line_with_prefix():
    body = get("/download?edition=hafs&format=txt&surah=112").text
...
def test_json_is_valid_and_carries_provenance():
    doc = get("/download?edition=hafs&format=json&surah=1").json()
    ...
    assert [a["ayah"] for a in doc["ayahs"]] == list(range(1, 8))


def test_json_nested():
    doc = get("/download?edition=hafs&format=json&nested=1&surah=113-114").json()
    assert [s["number"] for s in doc["surahs"]] == [113, 114]
    assert len(doc["surahs"][1]["ayahs"]) == 6
    assert "surah" not in doc["surahs"][0]["ayahs"][0]


def test_csv_parses():
    body = get("/download?edition=hafs&format=csv&surah=1&header=0").text
    rows = list(csv.reader(io.StringIO(body)))
    assert rows[0] == ["surah", "ayah", "text"]
    assert len(rows) == 8 and rows[1][:2] == ["1", "1"]


def test_xml_is_tanzil_compatible():
    body = get("/download?edition=hafs&format=xml&surah=112").text
    root = ET.fromstring(body)
    assert root.tag == "quran" and root.get("edition") == "hafs"
    surah = root.find("surah")
    assert surah.get("index") == "112" and surah.get("name")
    ayahs = surah.findall("ayah")
    assert [a.get("index") for a in ayahs] == ["1", "2", "3", "4"]
    assert ayahs[0].get("text").startswith("قُلۡ")


def test_sql_loads_into_sqlite():
    body = get("/download?edition=hafs&format=sql&surah=1&fields=surah,ayah,page").text
...
def test_md_has_surah_heading():
    body = get("/download?edition=hafs&format=md&surah=114").text
...
def ayah_text(**params):
    ...
    return get(f"/download?format=json&header=0&{q}").json()["ayahs"][0]["text"]
...
def test_pages_in_txt():
    body = get("/download?edition=hafs&format=txt&surah=1&pages=1&header=0").text
...
def test_fields_and_hafs_reference_for_warsh():
    doc = get("/download?edition=warsh&format=json&fields=surah,ayah,page,line,juz,hafs&ayah=2:253").json()
    a = doc["ayahs"][0]
...
def test_per_word_signs_attached_or_in_columns():
    ...
    division = get("/download?edition=hafs&format=json&by=word&ayah=2:26&signs=attached&header=0").json()["words"][0]
    assert division["text"].startswith("۞ ")
    plain = get("/download?edition=hafs&format=json&by=word&ayah=7:206&signs=attached&text=rasm_uthmani,plain&header=0").json()["words"][-1]
    assert plain["text"].endswith("۩") and not plain["plain"].endswith("۩")


def test_unnumbered_basmalah_is_ayah_zero_in_warsh():
    doc = get("/download?edition=warsh&format=json&surah=1").json()
    assert doc["ayahs"][0]["ayah"] == 0 and doc["ayahs"][0]["text"].startswith("بِسْمِ")
    assert len(doc["ayahs"]) == 8
    doc = get("/download?edition=hafs&format=json&surah=1").json()
    assert doc["ayahs"][0]["ayah"] == 1


def test_warsh_ayah_marks_render():
    body = get("/download?edition=warsh&format=txt&surah=1&markers=sign&header=0").text
    assert "۝٧" in body


def test_limit_previews():
    doc = get("/download?edition=hafs&format=json&limit=3").json()
    assert len(doc["ayahs"]) == 3 and doc["meta"]["limited"] is True
...
@pytest.mark.parametrize("url,fragment", [
    ...
    ("/download?edition=hafs&surah=115", "there are 114"),
    ("/download?edition=hafs&surah=3-2", "runs backwards"),
    ("/download?edition=hafs&page=605", "604 pages"),
    ("/download?edition=hafs&juz=31", "there are 30"),
    ("/download?edition=hafs&ayah=abc", "write surah:ayah"),
    ("/download?edition=hafs&surah=2&juz=1", "give one scope"),
...
def test_bazzi_has_no_juz():
    ...
    assert "no juz layer" in get("/download?edition=bazzi&fields=surah,ayah,juz", 400).json()["error"]
    assert get("/download?edition=bazzi&page=1").status_code == 200


def test_rasm_imlai_only_for_hafs():
    assert "Ḥafṣ only" in get("/download?edition=warsh&text=rasm_imlai", 400).json()["error"]
    assert get("/download?edition=hafs&text=rasm_imlai&surah=1").status_code == 200


# --- headers -----------------------------------------------------------------

def test_checksum_and_disposition():
    r = get("/download?edition=warsh&format=json&surah=1")
    assert r.headers["x-checksum-sha256"] == hashlib.sha256(r.content).hexdigest()
    assert r.headers["content-disposition"] == "attachment; filename*=UTF-8''quran-warsh-rasm_uthmani-surah1.json"
...
def test_editions():
    ...
    assert e["bazzi"]["has"]["juz"] is False and e["hafs"]["has"]["rasm_imlai"] is True
...
def test_several_text_forms_become_columns():
    r = get("/download?edition=hafs&ayah=1:1&text=rasm_uthmani,rasm_imlai,plain&format=csv&header=0")
    rows = list(csv.DictReader(io.StringIO(r.text)))
    assert list(rows[0].keys()) == ["surah", "ayah", "text", "rasm_imlai", "plain"]
    assert rows[0]["rasm_imlai"].startswith("بسم") and rows[0]["plain"].startswith("بسم")
    assert rows[0]["text"] != rows[0]["plain"]
    txt = get("/download?edition=hafs&ayah=1:1&text=rasm_uthmani,rasm_imlai&header=0").text.strip()
    assert txt.count("|") == 3                     # surah|ayah|rasm_uthmani|rasm_imlai
    j = get("/download?edition=hafs&ayah=1:1&text=rasm_imlai,rasm_uthmani&format=json&header=0").json()
    assert set(j["ayahs"][0]) == {"surah", "ayah", "text", "rasm_uthmani"}
    assert j["ayahs"][0]["text"].startswith("بسم")
    assert get("/download?edition=hafs&ayah=1:1&text=rasm_uthmani,rasm_uthmani&format=csv&header=0").text.splitlines()[0] == "surah,ayah,text"
    assert "Ḥafṣ only" in get("/download?edition=warsh&text=rasm_uthmani,rasm_imlai", 400).json()["error"]
    assert "text=" in get("/download?edition=hafs&text=rasm_uthmani,bogus", 400).json()["error"] or "bogus" in get("/download?edition=hafs&text=rasm_uthmani,bogus", 400).json()["error"]


def test_every_edition_has_a_sample_with_all_its_signs():
    for e in get("/editions").json():
        assert "sajdah" in e["sample"]["signs"] and "division" in e["sample"]["signs"], e["key"]
        if e["marks"].get("waqf"):
...
def test_map_dataset_by_ayah_and_by_word():
    ...
    assert d["meta"]["columns"] == ["surah", "ayah", "first_number", "last_number", "warsh_ayah", "warsh_relation"]
    d = get("/map?from=warsh&ayah=2:253").json()
    assert d["meta"]["to"] == ["hafs", "shubah", "qalun", "duri", "susi", "bazzi"]
    assert d["rows"][0]["hafs_ayah"] == "2:255" and d["rows"][0]["duri_relation"] == "merged"
    ...
    csv_ = get("/map?from=hafs&to=warsh,duri&surah=1&format=csv&header=0").text.splitlines()
    assert csv_[0] == "surah,ayah,first_number,last_number,warsh_ayah,warsh_relation,duri_ayah,duri_relation"
    assert len(csv_) == 8
    whole = get("/map?from=hafs&to=warsh&format=csv&header=0").text.splitlines()
    assert len(whole) == 6237                            # every Kūfī āyah, once
    r = get("/map?from=hafs&to=warsh&surah=1&format=sql&header=0")
