...
const ALL_KINDS = new Set(["waqf", "division", "sajdah"]);
...
export class WordIndex {
  ...
  search(text) {
    if (!this._byPlain) {
      ...
      for (const r of this._words) {
        const k = fold(r.rasm_uthmani);
        if (!this._byPlain.has(k)) this._byPlain.set(k, []);
        this._byPlain.get(k).push(r);
      }
    }
    return (this._byPlain.get(fold(text)) ?? []).map((r) => new IndexedWord(r));
  }
