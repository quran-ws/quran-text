...
import { Mushaf, AyahMap, WordIndex, ayahMarker, fold } from "./quran-text.js";
...
test("ayah text, layers and rendering", () => {
  ...
  assert.ok(a.render({ ayahMarkers: true }).endsWith(" ۝٢٥٥"));
...
test("sura, page, line, juz", () => {
  const s = hafs.sura(112);
  assert.equal(s.ayat.length, 4);
  assert.equal(s.render({ ayahMarkers: true }).split("۝").length, 5);
  assert.equal(s.basmalah, null);
  const p = hafs.page(3);
  assert.equal(p.lines.length, 15);
  assert.equal(p.ayat[0].key, "2:6");
  assert.equal(p.ayat.at(-1).key, "2:16");
  ...
  assert.equal(hafs.sura(2).lastPage.number, 49);
});
...
test("words, marks, numbering", () => {
  ...
  assert.equal(w.imlaei, "مالك");
  assert.equal(w.index, 1);
  assert.equal(hafs.sajdat().map((a) => a.key)[0], "7:206");
  assert.equal(hafs.sajdat().length, 15);
  assert.equal(hafs.hizbMarks().length, 199);
  assert.equal(hafs.hizbMarks()[0].render(), "۞ إِنَّ");
...
test("search", () => {
  ...
  assert.equal(ayahMarker(255), "۝٢٥٥");
});
...
test("bundled hafs", async () => {
  ...
  assert.equal(warsh.font.file, "out/fonts/UthmanicWarsh-v-3.0.ttf");
});

test("to(): the same ayah and word in another riwayah", () => {
  const m = hafs.ayah(2, 255).to(warsh);
  assert.equal(m.key, "2:253-254"); assert.equal(m.relation, "split"); assert.equal(m.ayat.length, 2);
...
test("ayah map", async () => {
  ...
  assert.deepEqual([r.sura, r.ayah, r.ayahLast, r.relation], [2, 253, 254, "split"]);
...
test("word index", async () => {
  ...
  assert.equal(idx.find(2, 255, 3).uthmani, "إِلَٰهَ");
