...
//     m.ayah(2, 255).text
//     m.ayah(2, 255).render(marks = MarkKind.all, ayahMarkers = true)
//     m.page(3).lines
...
fun ayahMarker(number: Int): String =
    END_OF_AYAH + number.toString().map { ARABIC_INDIC[it - '0'] }.joinToString("")
...
class Word internal constructor(private val m: Mushaf, val position: Int) {
    ...
    val imlaei: String? get() = m.imlaeiColumn?.get(position)
    val sura: Sura get() = m.suraAt(position)
...
open class Span internal constructor(internal val m: Mushaf, val start: Int, val end: Int) : Iterable<Word> {
     ...
     */
    fun render(marks: Set<MarkKind> = emptySet(), ayahMarkers: Boolean = false, lines: Boolean = false): String {
        val out = StringBuilder()
        for (pos in start until end) {
            if (lines && pos != start && pos in m.lineStartSet) out.append('\n')
            else if (pos != start) out.append(' ')
            var token = m.words[pos]
            for (mk in m.marksAt[pos] ?: emptyList()) {
                if (mk.kind !in marks) continue
                token = if (mk.side == MarkSide.before) mk.sign + " " + token else token + mk.sign
            }
            out.append(token)
            if (ayahMarkers) m.ayahEnds[pos]?.let { out.append(' ').append(ayahMarker(m.ayahNumber(it))) }
        }
        return out.toString()
    }

    /** Every numbered āyah with at least one word in the span. */
    open val ayat: List<Ayah>
        get() {
            val first = maxOf(indexOf(m.ayahStarts, start), 0)
            val last = indexOf(m.ayahStarts, end - 1)
            return (first..last).map { Ayah.fromIndex(m, it) }
        }
    val firstAyah: Ayah? get() = ayat.firstOrNull()
    val lastAyah: Ayah? get() = ayat.lastOrNull()

    val suras: List<Sura> get() = m.suras.subList(indexOf(m.suraStarts, start), indexOf(m.suraStarts, end - 1) + 1)
 ...
 */
data class AyahMatch(val ayat: List<Ayah>, val relation: String) {
    val first: Ayah? get() = ayat.firstOrNull()
    val last: Ayah? get() = ayat.lastOrNull()
    /** "2:253-254" */
    val key: String get() {
        val a = ayat.firstOrNull() ?: return ""
        val b = ayat.last()
...
class Ayah private constructor(m: Mushaf, val sura: Sura, val number: Int, /** 0-based ordinal in the muṣḥaf. */ val index: Int) :
    Span(m, m.ayahStarts[index], if (index + 1 < m.ayahStarts.size) m.ayahStarts[index + 1] else m.words.size) {

    companion object {
        internal operator fun invoke(m: Mushaf, sura: Int, number: Int): Ayah {
            val s = m.sura(sura)
        ...
        internal fun fromIndex(m: Mushaf, k: Int): Ayah {
            val s = m.suras[m.suraOfAyahIndex(k)]
            return Ayah(m, s, k - s.firstAyahIndex + 1, k)
        }
    }

    /** "2:255" */
    val key: String get() = "${sura.number}:$number"
    ...
    val imlaei: List<String?>? get() = m.imlaeiColumn?.subList(start, end)
    val hasSajdah: Boolean get() = marks.any { it.second.kind == MarkKind.sajdah }
    /** ۝٢٥٥ */
    val marker: String get() = ayahMarker(number)
...
class Sura internal constructor(m: Mushaf, val number: Int, info: JSONObject) :
    Span(m, m.suraStarts[number - 1], if (number < m.suraStarts.size) m.suraStarts[number] else m.words.size) {
    ...
    override val ayat: List<Ayah> get() = (1..ayahCount).map { Ayah(m, number, it) }
...
class Mushaf(private val doc: JSONObject) {
    ...
    val qariEn: String? = doc.getJSONObject("mushaf").optString("qari_en", null)
    val qariAr: String? = doc.getJSONObject("mushaf").optString("qari_ar", null)
    ...
    internal val imlaeiColumn: List<String?>? =
        doc.optJSONArray("imlaei")?.let { a -> List(a.length()) { if (a.isNull(it)) null else a.getString(it) } }
    internal val suraStarts = doc.getJSONArray("sura_starts").toIntArray()
    ...
    val suras: List<Sura> = (1..114).map { Sura(this, it, doc.getJSONArray("suras").getJSONObject(it - 1)) }
    private val suraFirstAyah = IntArray(114) { suras[it].firstAyahIndex }
    ...
    fun word(sura: Int, ayah: Int, index: Int): Word = Ayah(this, sura, ayah).word(index)
    ...
    val ayat: List<Ayah> get() = (0 until ayahCount).map { Ayah.fromIndex(this, it) }
    ...
    fun suraAt(position: Int): Sura = suras[indexOf(suraStarts, position)]
    ...
    fun hizbMarks(): List<Word> = positionsWith(MarkKind.hizb).map { Word(this, it) }
    ...
    internal fun suraOfAyahIndex(k: Int) = indexOf(suraFirstAyah, k)
    internal fun ayahNumber(k: Int) = k - suraFirstAyah[suraOfAyahIndex(k)] + 1
 ...
 */
data class AyahRef(val sura: Int, val ayah: Int, val relation: String, val ayahLast: Int? = null) {
    /** "2:253-254" */
    val key: String get() = if (ayahLast != null) "$sura:$ayah-$ayahLast" else "$sura:$ayah"
    override fun toString() = key
}
...
class AyahMap(doc: JSONObject) {
    ...
    private val rows: Map<String, JSONObject> = doc.getJSONArray("ayat").let { a ->
        (0 until a.length()).associate { i -> a.getJSONObject(i).let { "${it.getInt("sura")}:${it.getInt("ayah")}" to it } }
    }
    ...
    fun convert(sura: Int, ayah: Int, to: String): AyahRef {
        val row = rows["$sura:$ayah"] ?: throw IllegalArgumentException("$sura:$ayah is not a Kūfī āyah")
        val r = row.optJSONObject(to) ?: throw IllegalArgumentException("no edition '$to'; editions are $editions")
        return AyahRef(r.getInt("sura"), r.getInt("ayah"), r.getString("relation"), if (r.has("ayah_last")) r.getInt("ayah_last") else null)
    }

    /** The reference in every edition. */
    fun all(sura: Int, ayah: Int): Map<String, AyahRef> = editions.associateWith { convert(sura, ayah, it) }
}
...
class IndexedWord(val raw: JSONObject) {
    ...
    val sura: Int get() = raw.getInt("sura")
    val index: Int get() = raw.getInt("index")
    val key: String get() = raw.getString("key")
    val uthmani: String get() = raw.getString("uthmani")
    val simple: String get() = raw.getString("simple")
    ...
    val hafs: Triple<Int, Int, Int>? get() = raw.optJSONObject("hafs")?.let { Triple(it.getInt("sura"), it.getInt("ayah"), it.getInt("pos")) }
    ...
    override fun toString() = "$number $uthmani"
}
...
class WordIndex(doc: JSONObject) : Iterable<IndexedWord> {
    ...
    private var bySimple: Map<String, List<JSONObject>>? = null
    ...
    fun find(sura: Int, ayah: Int, index: Int): IndexedWord? {
        val by = byHafs ?: HashMap<String, JSONObject>().also { map ->
            for (i in 0 until records.length()) {
                val r = record(i)
                r.optJSONObject("hafs")?.let { h -> map.putIfAbsent("${h.getInt("sura")}:${h.getInt("ayah")}:${h.getInt("pos")}", r) }
            }
            byHafs = map
        }
        return by["$sura:$ayah:$index"]?.let { IndexedWord(it) }
    }

    /** Every number whose folded spelling equals [text], folded. */
    fun search(text: String): List<IndexedWord> {
        val by = bySimple ?: HashMap<String, MutableList<JSONObject>>().also { map ->
            for (i in 0 until records.length()) {
                val r = record(i)
                map.getOrPut(fold(r.getString("uthmani"))) { ArrayList(1) }.add(r)
            }
            bySimple = map
        }
