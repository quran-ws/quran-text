...
from quran_text import AyahMap, Mushaf, WordIndex, ayah_marker, fold  # noqa: E402
...
class TestAyah(unittest.TestCase):
    def test_text_layers_and_rendering(self):
        ...
        self.assertTrue(a.render(ayah_markers=True).endswith(" ۝٢٥٥"))
        self.assertIn("ۚ", a.render(marks=True))
        self.assertIn("ۚ", a.render(marks={"waqf"}))
        self.assertNotIn("ۚ", a.render(marks={"hizb"}))
    ...
    def test_sura_page_line_juz(self):
        s = hafs.sura(112)
        self.assertEqual(len(s.ayat), 4)
        self.assertEqual(s.render(ayah_markers=True).count("۝"), 4)
        self.assertIsNone(s.basmalah)
        p = hafs.page(3)
        self.assertEqual(len(p.lines), 15)
        self.assertEqual((p.ayat[0].key, p.ayat[-1].key), ("2:6", "2:16"))
        ...
        self.assertEqual(hafs.sura(2).last_page.number, 49)
        self.assertEqual([a.key for a in hafs.line(1, 3).ayat], ["1:3", "1:4"])

    def test_words_marks_numbering(self):
        w = hafs.word(1, 4, 1)
        self.assertEqual((w.text, w.number, w.imlaei, w.index), ("مَٰلِكِ", 11, "مالك", 1))
        ...
        self.assertEqual(len(hafs.hizb_marks()), 199)
        self.assertEqual(hafs.hizb_marks()[0].render(), "۞ إِنَّ")
    ...
    def test_search(self):
        ...
        self.assertEqual(ayah_marker(255), "۝٢٥٥")
...
class TestToAnotherRiwayah(unittest.TestCase):
    def test_ayah_and_word(self):
        ...
        self.assertEqual((m.key, m.relation, len(m.ayat)), ("2:253-254", "split", 2))
    ...
    def test_agrees_with_ayah_map_for_sura_2(self):
        rows = [r for r in AyahMap.load(OUT / "ayah-map.json")._rows.values() if r["sura"] == 2]
        for r in rows:
            cell = r["warsh"]
...
class TestAyahMap(unittest.TestCase):
    def test_convert(self):
        ...
        self.assertEqual((r.sura, r.ayah, r.ayah_last, r.relation), (2, 253, 254, "split"))
...
class TestWordIndex(unittest.TestCase):
    def test_lookups(self):
        ...
        self.assertEqual(idx.find(2, 255, 3).uthmani, "إِلَٰهَ")
