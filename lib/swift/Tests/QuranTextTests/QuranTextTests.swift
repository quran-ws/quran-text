...
final class QuranTextTests: XCTestCase {
    ...
    func testAyahTextLayersAndRendering() {
        ...
        XCTAssertTrue(a.render(ayahMarkers: true).hasSuffix(" ۝٢٥٥"))
        // Pause signs are combining marks: look at unicodeScalars, not Characters.
        ...
        XCTAssertFalse(a.render(marks: [.hizb]).unicodeScalars.contains("\u{06DA}"))
    ...
    func testSuraPageLineJuz() {
        let s = hafs.sura(112)
        XCTAssertEqual(s.ayat.count, 4)
        XCTAssertEqual(s.render(ayahMarkers: true).unicodeScalars.filter { $0 == "\u{06DD}" }.count, 4)
        XCTAssertNil(s.basmalah)
        let p = hafs.page(3)
        XCTAssertEqual(p.lines.count, 15)
        XCTAssertEqual([p.ayat.first!.key, p.ayat.last!.key], ["2:6", "2:16"])
        ...
        XCTAssertEqual(hafs.sura(2).lastPage.number, 49)
        XCTAssertEqual(hafs.line(1, 3).ayat.map { $0.key }, ["1:3", "1:4"])
    }
    ...
    func testWordsMarksNumbering() {
        ...
        XCTAssertEqual(hafs.hizbMarks().count, 199)
        XCTAssertEqual(hafs.hizbMarks()[0].render(), "۞ إِنَّ")
    ...
    func testSearch() {
        ...
        XCTAssertEqual(ayahMarker(255), "۝٢٥٥")
    }
    ...
    func testAyahMap() throws {
        ...
        XCTAssertEqual(r, AyahRef(sura: 2, ayah: 253, relation: "split", ayahLast: 254))
    ...
    func testWordIndex() throws {
        ...
        XCTAssertEqual(idx.find(2, 255, 3)?.uthmani, "إِلَٰهَ")
