...
Every number carries its Ḥafṣ coordinates ``{sura, ayah, pos}`` in the Kūfī
...
CSV_COLUMNS = (["number", "sura", "index", "key", "rasm", "pointed", "uthmani",
                "simple", "status", "hafs_sura", "hafs_ayah", "hafs_pos",
...
def _csv_row(w: Word, hafs: dict[int, dict]) -> list:
    h = hafs.get(w.id)
    return ([w.id, w.sura, w.index, w.key, w.rasm, w.pointed, w.uthmani, w.simple,
...
def write_differences(words: list[Word]) -> dict:
    ...
    with (OUT / "differences.csv").open("w", encoding="utf-8", newline="") as fh:
        wr = csv.writer(fh)
        wr.writerow(["number", "sura", "index", "ayah_hafs", "status", "rasm",
        ...
        for w in flagged:
            ...
            wr.writerow([
                ...
                len(spellings),
                "  ||  ".join(f"{g['text']} [{','.join(g['riwayat'])}]" for g in spellings),
            ])
    return doc
...
def boundary_events(words: list[Word]) -> list[dict]:
    ...
    the muṣḥaf's own orthography, carried in ``numbering`` and the word index,
