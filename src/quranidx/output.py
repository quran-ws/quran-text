...
def _word_json(w: Word) -> dict:
    """One word.  ``forms`` always lists every riwāyah that has the word, so a
    consumer never has to fall back to another record to resolve a spelling."""
    rec = {
        "id": w.id,
        "i": w.index,
        "key": w.key,
...
def write_all(words: list[Word], riwayat: list[Riwaya]) -> dict:
    ...
    meta = {
        ...
        "model": (
            "A sūrah is a flat list of words.  The āyah number is an attribute "
            "of a word, not a level of nesting, because the riwāyāt disagree "
            "about where āyāt end but agree about the sequence of words."
        ),
    }
    ...
    with (OUT / "words.csv").open("w", encoding="utf-8", newline="") as fh:
        wr = csv.writer(fh)
        wr.writerow(["word_id", "sura", "word_index", "key", "rasm", "uthmani",
                     "simple", "status", "present_count"]
                    + [f"aya_{k}" for k in keys] + [f"form_{k}" for k in keys])
        for w in words:
            wr.writerow([w.id, w.sura, w.index, w.key, w.rasm, w.uthmani,
                         w.simple, w.status, len(w.present)]
    ...
    with (OUT / "conflicts.csv").open("w", encoding="utf-8", newline="") as fh:
        ...
        for w in flagged:
            ...
            wr.writerow([
                ...
                "|".join(w.missing), "|".join(sorted(w.boundary)),
                len(groups),
