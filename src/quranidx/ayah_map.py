...
def _runs(words: list[Word], key: str) -> dict[tuple[int, int], set[int]]:
    """``(sura, ayah)`` in one edition -> the numbers it contains."""
    out: dict[tuple[int, int], set[int]] = defaultdict(set)
    for w in words:
        if key in w.aya:
            out[(w.sura, w.aya[key])].add(w.id)
    return out
...
def ayah_map(words: list[Word]) -> list[dict]:
    ...
    out = []
    for (sura, ayah), numbers in sorted(reference.items()):
        entry: dict = {"sura": sura, "ayah": ayah}
        for k in ORDER:
            ...
            cell = {"sura": first[0], "ayah": first[1]}
            if last != first:
...
def write_ayah_map(words: list[Word]) -> dict:
    ...
    with (OUT / "ayah-map.json").open("w", encoding="utf-8") as fh:
        head = {k: v for k, v in doc.items() if k != "ayat"}
    ...
    with (OUT / "ayah-map.csv").open("w", encoding="utf-8", newline="") as fh:
        wr = csv.writer(fh)
        wr.writerow(["sura", "ayah"] + ORDER + [f"{k}_relation" for k in ORDER])
        for r in rows:
            wr.writerow([r["sura"], r["ayah"]]
